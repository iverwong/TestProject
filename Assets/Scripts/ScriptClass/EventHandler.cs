...
public static class EventHandler
{
    ...
    public delegate float RoleEventHandler(BaseInteractableObject _interactableObject, float _value);
    /// <summary>
    /// 过滤器委托类，对目标列表产生影响，用于过滤掉无法造成影响的目标
    /// </summary>
    /// <param name="_objectList">从目标触发器获得的目标清单</param>
    /// <returns>过滤后返回的目标清单</returns>
    public delegate List<BaseInteractableObject> FilterEventHandler(List<BaseInteractableObject> _objectList);
    /// <summary>
    ...
    public static float Action(BaseInteractableObject _interactableObject,float _value, RoleEventHandler _event)
    {
        if(_event != null)
        {
            //获取委托链表
            ...
            foreach(System.Delegate each in delegates)
            {
                ...
                //将_value的值更改为运行委托后的值
                _value = handler(_interactableObject, _value);
            }
            return _value;//返回最终值
        }
    ...
    public static List<BaseInteractableObject> Filter(List<BaseInteractableObject> _objectList, FilterEventHandler _event)
    {
        if (_event != null)
        {
            //获取委托链表
            ...
            foreach (System.Delegate each in delegates)
            {
                ...
                //将_value的值更改为运行委托后的值
                _objectList = handler(_objectList);
            }
